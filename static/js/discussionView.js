...
for (var i = 0; i < commentEditButtons.length; i++){
    commentEditButtons[i].addEventListener('click', function(e){
        ...
        URLstring = "/discussion/" + commentEntryForm.discussionID.value + "/comment/" + actionID + "/edit#commentItemTextDiv" + actionID;
...
for (var i = 0; i < commentEditButtons.length; i++){
    commentDeleteButtons[i].addEventListener('click', function(e){
        actionID = parseInt(this.id.replace(/[^0-9\.]/g, ''), 10);
        URLstring = "/discussion/" + commentEntryForm.discussionID.value + "/comment/" + actionID + "/delete";
...
function discussionDeletePrompt(myID){
    // this function is called from an "onclick" reference in the HTML tag of the discussion delete button
    actionID = parseInt(myID.replace(/[^0-9\.]/g, ''), 10);
    URLstring = "/discussion/delete/" + actionID;
    if (window.confirm("Are you sure you want to delete this discussion, and any comments that follow?")) {
        window.open(URLstring,"_self");
    } else {
        return false;
    }
